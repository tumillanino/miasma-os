...
---@alias OscArgument { name: string?, type: "boolean"|"number"|"string" }
...
---@param type "boolean"|"number"|"string"
---@return OscArgument
...
---@class OscAction
---@field pattern string OSC message pattern like "/transport/start".
---@field description string? Optional string which describes the action.
---@field arguments (OscArgument)[]? Optional array of arguments (see function 'argument').
---@field handler fun(...: boolean|number|string) Function which applies the action.

--- Register a global Renoise OSC message:
---@param map table
---@param info OscAction
...
---@param value any
---@param dest_type "string"|"number"|"boolean"
...
add_global_action { 
  pattern = "/song/bpm", 
  description = "Set the songs current BPM [20 - 999]",
  
  arguments = { argument("bpm", "number") },
  handler = function(bpm)
    song().transport.bpm = clamp_value(bpm, 20, 999)
  end,
}
