...
The message parameter, passed to invoke the action, is defined as:
 ...
 class "TriggerMessage"
  -- returns if the message should invoke something
...
scripting terminal. Please visit https://scripting.renoise.com for more info
about this.
...
add_action("Transport:Edit:MIDI Octave Enabled [Toggle]",
function(message)
  song().transport.octave_enabled = toggle_message_value(
    message, song().transport.octave_enabled)
end)

add_action("Transport:Edit:MIDI Octave Enabled [Set]",
function(message)
  song().transport.octave_enabled = boolean_message_value(
    message, song().transport.octave_enabled)
end)
...
add_action("GUI:Show Right Frame [Trigger]",
function(message)
  if message:is_trigger() then
    app().window.right_frame_is_visible =
      not app().window.right_frame_is_visible
  end
end)

add_action("GUI:Show Right Frame [Set]",
function(message)
  app().window.right_frame_is_visible = boolean_message_value(
    message, app().window.right_frame_is_visible)
end)
