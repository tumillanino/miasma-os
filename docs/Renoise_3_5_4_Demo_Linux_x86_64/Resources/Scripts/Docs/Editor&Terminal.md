...
The built-in `Script Editor and Terminal` allows you to:

* Create, view and edit Lua, text, Markdown and and XML files.
...
+ `Command + K`  Comment/Uncomment Lines
+ `Command + /`  ^
...
+ `Command + Z` Undo
+ `Command + NUMPAD+`  Increase Text Size
+ `Command + I`        ^^
+ `Command + NUMPAD-`  Decrease Text Size
+ `Command + U`        ^^
+ `F3` Find Next
...
* To enter multiple lines in the terminal, use Shift + Return

* `oprint(some_object)` prints out info about a Lua class object:
  Try for example `oprint(renoise.song())` to see all properties
  and methods for the Renoise song. To list all available modules, try 
  `rprint(renoise)` or `rprint(_G)`
